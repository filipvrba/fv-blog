...
export default class CDatabase {
  ...
  updateArticle(options, callback) {
    ...
    let fullText = options.text.encodeBase64();
    let isAdult = options.isAdult ? 1 : 0;
    let query = `UPDATE articles SET file_id = ${fileId}, title = '${title}', category = '${category}', short_text = '${shortText}', full_text = '${fullText}', is_adult = ${isAdult}, changed_at = CURRENT_TIMESTAMP WHERE id = ${this._parent.articleId};`;
