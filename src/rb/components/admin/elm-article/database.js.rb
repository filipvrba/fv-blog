...
export default class CDatabase
  ...
  def save_article(options, &callback)
    ...
    query = "INSERT INTO articles (user_id, file_id, title, category, short_text, full_text) " +
  ...
  def update_article(options, &callback)
    ...
    query = "UPDATE articles SET file_id = #{file_id}, title = '#{title}', " +
      ...
      "full_text = '#{full_text}' WHERE id = #{@parent.article_id};"
  ...
  def get_article(&callback)
    query = "SELECT file_id, title, category, full_text " +
